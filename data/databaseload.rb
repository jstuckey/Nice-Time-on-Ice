...
# Check for scrape file passed in
if ARGV[0]
  # Get the scrape file name
  scrapeFileName = ARGV[0]
  ...
  if Dir.pwd.include? "/data"
    ...
    scrapeFile = File.open(scrapeFileName)
  else
    # Open game scrape file from data directory
    scrapeFile = File.open(File.expand_path("data/#{scrapeFileName}", Dir.pwd)) 
  end
